# code to calculate unique steady states & number of stable steady states of non-linear ODEs with simgoidal lambda.
import os
...
for nStep in range(len(nSteps)):
    ...
    for aStep in range(len(aSteps)):
        ...
        for bStep in range(len(bSteps)):
            ...
            for step in range(len(energySteps)):
                ...
                # populate unique steady states dataframe
    ...
    csvFileName = "data-files/sigmoid-n%.0f-unique-steady-states.csv" % n
    export_Dataframe = uniqueSSDataframe.to_csv(csvFileName, index=False, header=True)

    csvFileName2 = "data-files/sigmoid-n%.0f-number-stable.csv" % n
    export_Dataframe2 = numStableSSDataframe.to_csv(
        csvFileName2, index=False, header=True)
