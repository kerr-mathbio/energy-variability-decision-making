# code to calculate unique steady states & number of stable steady states of non-linear ODEs with modified simgoidal lambda.
import os
    ...
    for bStep in range(len(bSteps)):
        ...
        for step in range(len(energySteps)):
            ...
            numStableSSDataframe.loc[len(numStableSSDataframe)] = [a, b, k, n, thetaA, thetaB,
                                                                   energy, len(uniqueSteadyStatesList),
                                                                   numStableMatches]

# export dataframes to csv files (without index col)
csvFileName = "data-files/sigmoid-modified-number-stable.csv"
export_Dataframe = numStableSSDataframe.to_csv(csvFileName, index=False, header=True)
